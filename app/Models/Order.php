    ...
    public function invoice($des = 'D')
    {
        ...
        foreach ($this->inventories as $item) {
            ...
            if ($this->cancellation && $this->cancellation->isItemInRequest($item->id))
            {
                ...
                $cancel_retunr_declienditems = explode(',',$this->cancellation->decline_item);
                $cancel_request_status = $this->cancellation->status;
                // print_r($cancel_exchange_retunr_option);

                if(array_key_exists($item->id,$cancel_exchange_retunr_option) && ($cancel_exchange_retunr_option[$item->id] == 'exchange') && ((($cancel_request_status== 1) || ($cancel_request_status== 2))))
                {
                    $exchange_return_option = 'Exchange Requested';
                }
                else  if(array_key_exists($item->id,$cancel_exchange_retunr_option) && ($cancel_exchange_retunr_option[$item->id] == 'refund') && ((($cancel_request_status== 1) || ($cancel_request_status== 2))))
                {
                    $exchange_return_option = 'Refund Requested';
                }
                else if(array_key_exists($item->id,$cancel_exchange_retunr_option) && ($cancel_exchange_retunr_option[$item->id] == 'exchange') && ($cancel_request_status== 6) && in_array($item->id,$cancel_retunr_approveditems) )
                {
                    $exchange_return_option = 'Exchange Successful';
                }
                else if(array_key_exists($item->id,$cancel_exchange_retunr_option) && ($cancel_exchange_retunr_option[$item->id] == 'refund') && ($cancel_request_status== 6) && in_array($item->id,$cancel_retunr_approveditems) )
                {
                    $exchange_return_option = 'Refund Successful';
                }


                else if(array_key_exists($item->id,$cancel_exchange_retunr_option) && ($cancel_exchange_retunr_option[$item->id] == 'exchange') && ($cancel_request_status== 6) && in_array($item->id,$cancel_retunr_declienditems) )
                {
                    $exchange_return_option = 'Exchange Declined';
                }
                else if(array_key_exists($item->id,$cancel_exchange_retunr_option) && ($cancel_exchange_retunr_option[$item->id] == 'refund') && ($cancel_request_status== 6) && in_array($item->id,$cancel_retunr_declienditems) )
                {
                    $exchange_return_option = 'Refund Declined';
                }
