                ...
                <div class="input-group full-width space30">
                    ...
                    <ul class="shopping-cart-summary ">
                        ...
                        <li>
                            ...
                            <span id="summary-total{{ $cart->id }}">{{ number_format($cart->total, 2, '.', '') }}</span>{{ get_currency_suffix() }}</span>
                        </li>
                        {{-- Discount --}}
                        <li id="discount-section-li{{ $cart->id }}disc">
                            ...
                            <span>{{ get_currency_prefix() }}
                                <span id="summary-discount{{ $cart->id }}disc">{{ $cart->coupon ? number_format($cart->discount, 2) : number_format(0, 2,)}}</span>{{ get_currency_suffix() }}
                            </span>
                        </li>
                        {{-- Sub Total --}}
                        <li>
                            <span>{{ trans('theme.subtotal') }}</span><span>{{ get_currency_prefix() }}
                        ...
                        <li>
                            <span>GST</span>
                            <span>{{ get_currency_prefix() }}
                            <span id="summary-total{{ $cart->id }}" class="item-total{{ $cart->id }}">{{ $gst_amount }}</span>{{ get_currency_suffix() }}</span>
                        </li>
                        <li>
                            <span><a class="dynamic-shipping-rates" data-toggle="popover" data-cart="{{ $cart->id }}"data-options="{{ $shipping_options[$cart->id] }}"id="shipping-options{{ $cart->id }}" title="{{ trans('theme.shipping') }}"><u>{{ trans('theme.shipping') }}</u></a>{{-- <em id="summary-shipping-name{{ $cart->id }}" class="small text-muted"></em> --}}</span>
                            <span>{{-- {{ get_currency_prefix() }}<span id="summary-shipping{{ $cart->id }}">{{ number_format($cart->get_shipping_cost(), 2, '.', '') }}</span>{{ get_currency_suffix() }} --}}Free</span>
                        </li>
                        {{-- @unless (empty(json_decode($packaging_options)))
                            <li>
                                <span><a class="packaging-options" data-toggle="popover" data-cart="{{ $cart->id }}" data-options="{{ $packaging_options }}" title="{{ trans('theme.packaging') }}"><u>{{ trans('theme.packaging') }}</u></a>
                                    <em class="small text-muted" id="summary-packaging-name{{ $cart->id }}">{{ optional($default_packaging)->name }}</em>
                                </span>
                                <span>{{ get_currency_prefix() }}<span id="summary-packaging{{ $cart->id }}">{{ number_format($default_packaging ? $default_packaging->cost : 0, 2, '.', '') }}</span>{{ get_currency_suffix() }}</span>
                            </li>
                        @endunless --}}
                        {{-- <li id="tax-section-li{{ $cart->id }}" style="display: {{ $cart->taxes < 0 ? 'block' : 'none' }};">
                                <span>{{ trans('theme.taxes') }}</span>
                                <span>{{ get_currency_prefix() }}<span id="summary-taxes{{ $cart->id }}">{{ number_format($cart->taxes, 2, '.', '') }}</span>{{ get_currency_suffix() }}</span>
                        </li> --}}
                        <li>
                            <span class="lead">{{ trans('theme.order_total') }}</span>
                            <span class="lead">{{ get_currency_prefix() }}
                            <span id="summary-grand-total{{ $cart->id }}12">{{ number_format($order_total, 2, '.', '') }}</span>{{ get_currency_suffix() }}</span>
                        </li>
                    </ul>
                ...
                <div class="col-md-5">
                    ...
                    @if (isset($customer))
                        ...
                        <div class="col-sm-12 space20">
                            <a href="{{ route('my.address.create') }}" class="modalAction btn btn-default btn-sm flat pull-right"><i class="fas fa-address-card-o"></i> @lang('theme.button.add_new_address')</a>
                        </div>
                    @else
